...
public class tk2dUIToggleButtonGroup : MonoBehaviour
{
	...
	/// <summary>
	/// Whether or not the button group will call the OnChange event when starting up.
	/// </summary>
	[SerializeField]
    ...
    private void ButtonToggle(tk2dUIToggleButton toggleButton)
    {
        if (toggleButton == null || toggleButton.IsOn)
        {
            ...
            if (toggleButton != selectedToggleButton)
            {
				...
				// IF this is the first toggle (from the Awake method), we can suppress the event dispatch
